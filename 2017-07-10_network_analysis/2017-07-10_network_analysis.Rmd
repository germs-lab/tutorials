...
```
## NOTE:
Many people also like to use `GGally`, `intergraph`, and `ggnet2` for generating network plots with more of a ggplot feel. I started there and found them making network plotting more confusing and lots of times unecessary. Therefore, I am not introducing them, but you are more than welcome to explore [here](https://briatte.github.io/ggnet/)
...
plot(temp.simplified, edge.width=E(temp.simplified)$weight * 4, layout=layout_in_circle, edge.lty = E(temp.simplified)$lty)
```
...
plot(temp.simplified, edge.width=E(temp.simplified)$weight * 4, layout=layout_in_circle, edge.lty = E(temp.simplified)$lty, edge.color = E(temp.simplified)$colors)
```
...
plot(temp.simplified, edge.width=E(temp.simplified)$weight * 4, layout=layout_in_circle, edge.lty = E(temp.simplified)$lty, edge.color = E(temp.simplified)$colors, vertex.color = V(temp.simplified)$colors)
```

#### 4.2.3. we can remove the node labels and add a legend:
```{r}
plot(temp.simplified, edge.width=E(temp.simplified)$weight * 4, layout=layout_in_circle, edge.lty = E(temp.simplified)$lty, edge.color = E(temp.simplified)$colors, vertex.color = V(temp.simplified)$colors, vertex.label=NA)
...
plot(temp.simplified, edge.width=E(temp.simplified)$weight * 4, layout=layout_in_circle, edge.lty = E(temp.simplified)$lty, edge.color = E(temp.simplified)$colors, vertex.color = V(temp.simplified)$colors, vertex.frame.color = V(temp.simplified)$frame.color, vertex.label=NA)
...
### 5.7. effective size: 
the number of relationships between a node and other unrelated nodes OR
